...
class TransactionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $tech_personnels = TechnicalPersonnel::where('project_id', $projectId)->get();

        // Retrieve project details using the project ID
        $projectDetail = Project::where('project_id', $projectId)->first();

        // Eager load particulars with materials, labors, and equipments
        $particulars = ProjectParticular::with([
            'materials' => function ($query) {
                $query->select('project_particular_materials.*', 'materials.*', 'material_categories.*', 'prices.*')
            ...
            'labors' => function ($query) {
                $query->select('project_particular_labors.*', 'labors.*', 'labor_rates.*')
            ...
            'equipments' => function ($query) {
                $query->select('project_particular_equipments.*', 'equipments.*', 'equipment_rates.*', 'equipment_categories.*')
                    ...
                    ->leftJoin('equipment_categories', 'equipments.equipment_category_id', '=', 'equipment_categories.equipment_category_id');
            },
        ])
            ->select('project_particulars.*', 'particulars.particular_name', 'particulars.pay_item')  // Include particulars data
            ->leftJoin('particulars', 'project_particulars.particular_id', '=', 'particulars.particular_id')
            ->where('project_id', $projectId)
            ->paginate(8);

        return view('transactions', compact('particulars', 'projectDetail', 'minimum_equipments', 'tech_personnels'));
    }
